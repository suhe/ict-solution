...
@section('content')
	<div class="row bg-title">
        ...
        <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
			...
			<a href="{!! url('/payment-method/form') !!}" class="btn btn-primary btn-rounded pull-right m-l-20 btn-sm  hidden-xs hidden-sm waves-effect waves-light"><i class="fa fa-pencil"></i> {!! Lang::get('app.create') !!}</a>
			@endif
			{!! Form::open(['url' => '/payment-method','method'=>'GET','class'=>'form-inline pull-right']) !!}
								...
								<td class="text-center">
									<div class="btn-group dropup m-r-10">
										...
										<ul class="dropdown-menu">
											<li><a href="{!! url('/payment-method/view/'.Crypt::encrypt($row->id)) !!}"> {!! Lang::get('app.view') !!}</a></li>
											@if(Role::access('u','payment-method'))
											<li><a href="{!! url('/payment-method/form/'.Crypt::encrypt($row->id)) !!}"> {!! Lang::get('app.edit') !!}</a></li>
											@endif
											@if(Role::access('d','payment-method'))
												@if($row->id != 1)
    ...
    $('.delete').on('click', function(event) {
		...
		$.confirm({
			...
			confirm: function(){
				$.ajax({
					type  : "post",
					url   : "{!! url('/payment-method/do-delete') !!}",
					data  : {id:id},
					dataType: "json",
					cache : false,
