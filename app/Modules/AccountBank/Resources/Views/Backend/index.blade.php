...
@section('content')
	<div class="row bg-title">
        ...
        <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
			...
			<a href="{!! url('/account-bank/form') !!}" class="btn btn-primary btn-rounded pull-right m-l-20 btn-sm  hidden-xs hidden-sm waves-effect waves-light"><i class="fa fa-pencil"></i> {!! Lang::get('app.create') !!}</a>
			@endif
			{!! Form::open(['url' => '/account-bank','method'=>'GET','class'=>'form-inline pull-right']) !!}
				...
				<table class="table">
					<thead>
						<tr>
							...
							<th class="col-md-2">@sortablelink('account_no', Lang::get('app.account no'))</th>
							<th class="col-md-3">@sortablelink('account_name', Lang::get('app.account name'))</th>
							<th class="col-md-2">@sortablelink('bank_id', Lang::get('app.bank'))</th>
							<th class="col-md-1 text-center">{!! Lang::get('app.status') !!}</th>
							<th class="col-md-1 text-center">{!! Lang::get('app.edit') !!}</th>
						</tr>
					</thead>
					<tbody>
						@foreach($account_banks as $key => $row)
							<tr class="row-{!! $row->id !!}">
								...
								<td class="text-center">
									<div class="btn-group dropup m-r-10">
										...
										<ul class="dropdown-menu">
											<li><a href="{!! url('/account-bank/view/'.Crypt::encrypt($row->id)) !!}"> {!! Lang::get('app.view') !!}</a></li>
											@if(Role::access('u','account-bank'))
											<li><a href="{!! url('/account-bank/form/'.Crypt::encrypt($row->id)) !!}"> {!! Lang::get('app.edit') !!}</a></li>
											@endif
											@if(Role::access('d','account-bank'))
												@if($row->id != 1)
