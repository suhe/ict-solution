	...
	<div class="row bg-title">
        ...
        <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
			@if(Role::access('c','customer-group'))
				<a href="{!! url('/customer-group/form') !!}" class="btn btn-primary btn-rounded pull-right m-l-20 btn-sm  hidden-xs hidden-sm waves-effect waves-light"><i class="fa fa-pencil"></i> {!! Lang::get('app.create') !!}</a>
			@endif
			{!! Form::open(['url' => '/customer-group','method'=>'GET','class'=>'form-inline pull-right']) !!}
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="white-box">
				<table class="table">
					<thead>
						<tr>
							...
							<th class="tcol-md-9">@sortablelink('name', Lang::get('app.name'))</th>
							<th class="text-center col-md-1">{!! Lang::get('app.status') !!}</th>
							<th class="text-center col-md-1">{!! Lang::get('app.edit') !!}</th>
						 </tr>
					</thead>
					<tbody>
						@foreach($customer_groups as $key => $row)
							<tr class="row-{!! $row->id !!}">
								...
								<td class="text-center">
									<div class="btn-group dropup m-r-10">
										...
										<ul class="dropdown-menu">
											<li><a href="{!! url('/customer-group/view/'.Crypt::encrypt($row->id)) !!}"> {!! Lang::get('app.view') !!}</a></li>
											@if(Role::access('u','customer-group'))
											<li><a href="{!! url('/customer-group/form/'.Crypt::encrypt($row->id)) !!}"> {!! Lang::get('app.edit') !!}</a></li>
											@endif
											@if(Role::access('d','customer-group'))
				...
				{!! $customer_groups->appends(Request::except('page'))->render() !!}
			</div>
		</div>
	</div>
@endsection
    ...
    $('.delete').on('click', function(event) {
		...
		$.confirm({
			...
			confirm: function(){
				$.ajax({
					type  : "post",
					url   : "{!! url('/customer-group/do-delete') !!}",
					data  : {id:id},
					dataType: "json",
					cache : false,
